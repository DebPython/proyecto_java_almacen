...
public class UsuarioDAOimpl extends ConexionDB implements UsuarioDAO {
    ...
    public void insert(Usuario usuario) throws Exception {
        try {
            ...
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("INSERT INTO usuarios(usuario,password) VALUES(?,sha1(?))");
    ...
    public void update(Usuario usuario) throws Exception {
        try {
            ...
            this.conectar();
            PreparedStatement ps = this.conn.prepareStatement("UPDATE usuarios SET usuario=?,password=sha1(?) WHERE id_usuario=?");
            ps.setString(1, usuario.getUsuario());
            ps.setString(2, usuario.getPassword());
